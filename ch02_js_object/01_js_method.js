...
/*
  8 번 라인에 대한 해석을 위해서는 어제 배운 함수 펴현식 관련한 부분을 알아야 겟네요.

  JS에서는 함수를 변수에 저장할 수 있었습니다.
  그래서  객체 명.()함수명()으로 방식으로는 method라고 할 수 있겠습니다.
*/
...
/*
  ...
  즉, slice()의 두 번 째 매개변수를 for문에서의 한계값 설정할 때 제가 이상/이하보다는 초과 / 미만을 사용하는 경우가 많은데, 그 이유가 여기서도 적용됩니다...
*/
let result4 = str2.slice(7);
console.log(result4); // 결과값 : banana Kiwi
/*
  매개변수를 하나만 ㄹ지정했을 경우 그 시작 인덱스로부터 문자열 끝까지 출력해줍니다.

  마이너스인덱스
  맨마지막 문자열의 index를 -1로 잡고 앞으로 올때마다 -1 씩 더해줍니다.
*/
let result5 = str2.slice(-12);
console.log(result5); // 결과값 : Banana, Kiwi
/*
  그러면 굳이 마이너스 인덱스를 써야하는 경우를 가정하겟습니다.
...
// let result7 = str2.substring(-12);  // 결과값 : Apple, Banana, Kiwi
// console.log(result7);
...
// 결과값 :
/*
  83 번 라인의 경우 처음 보는 형태 입니다.
  JS 정규식 표현식을 썼는데 추후 설명 예정이고 i는 insensitive의 축약어로 덜 깐깐하게 굴겟다는 겁니다.
  대소문자 구분 안하고 그냥 찾는다는 의미.
*/
let rpdStc3 = eStc.replace(/Seoul/g, 'Busan');
console.log(rpdStc3);
// 결과값 : Please visit Busan and Busan
// /g는 global약자로, 맨 앞에 것만
...
/*
  실무 사용 예시
  우리나라에서는 이름이 성 이름이 붙어있지만 다른ㄹ나라에서는 이름 성 형태나 / 성 이름 에서 공백으로 구분하는 경우가 있습니다. 이상의 경우에 
