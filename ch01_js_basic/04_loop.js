...
// // 이상과 같은 방식을 굳이 할 필요 없으니깐 반복문 써야겟죠
// // Java와 동일합니다.
...
/*
  ...
  첫 번째, 마지막 index에 해당하는 element다음에도 ','가 찍혀있는데도 불구하고 오류가 발생하지 않는다는 점

  두 번재, Array의 element인 object의 마지막 property의 value 값 다음에도 ','가 있지만 오류 발생하지 않는다는 점
...
// console.log(productList[4].name); // 그럼 Array의 element는 object죠.
...
// // ; 콜론 안해도 원래 오류가 안납니다. 근데 하는게 매너입니다.

// let inputCoin = 900;
// let outputList = []; // 빈 배열을 하나 선언하겠습니다.
// // 이제 inputCoin으로 살 수 있는 음료 목록들을 집어 넣도록 할겁니다.
...
// }
// 근데 list 내부의 객체들을 하나씩 확인하려고 하니까 얘는 또 출력이되비다.

/*
  즉 이상의 내용을 정리해봤을 때, 배열 내부에 객체가 element라면, 배열 자체를 console.log(배열명); 으로 출력했을 때 결과값이 [object Object],[object Object],[object Object],[object Object]
  형식으로만 나오게 되고,

  각 겍체의 명확한 상태를 알기 위해서는 결국 배열의 각 element를 추출하는 반복문을 작성해야 한다는 점을 확인할수 있습니다.
*/
...
/*
  outPutList의 element의 자료형은 객체이고, 객체에는 index number가 존재하지 ㅏㄶ으므로 이중 for문을 통해서 객체 내부의 각 property를 확인하는 것은 불가능합니다.
...
// 그래서 해당 key에 할당된 value를 추출하고 싶다면 121 번 라인과 같은 방식으로 코드를 작성해야 합니다.
for(const key in person) {  // 그럼 추출되는 key는 각각 fName / lname / age 일겁니다.
  console.log(key);
  console.log(person[key]);
  // 이 시점에서 첫 번재 반복이라고 가정하겠습니다.
  // 그러면 key = 'fname'이 저장되어있다고 볼 수 있겟네요

  // 즉 person.key는 person.fName이 아니라 person. 'fName'이라는 점이 문제 입니다.
  // console.log(person.key);
}
// 그러면 console.log(person.key);는 불가능할까요?

// 그래서 굳이 # 1, 2로 두 가지 방식을 통해서 객체의 value를 조회할수 있었습니다.
...
// 객체의 property의 key를 추출 할 수 있으니까, 마치 향상된 for문 처럼 여겨져서
// 배열을
// for(let num in nums) {
//   console.log(num)
...
// 일, 반적인 Java에서의 enhanced for문에 해당되는 것은 for-of문 이라고 생각해주시면 좋겠습니다.
