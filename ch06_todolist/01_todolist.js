...
function renderTodos() {
  ...
  todos.forEach((todo, index) => {
    ...
    // 요소들을 li 변수에 추가해야 합니다. 왜? li내부에 체그박스 / 스팬 / 버튼
    li.append(checkbox);
    li.append(span);
    li.append(deleteBtn);
    ...
    checkbox.addEventListener('change', () => {
      todos[index].completed = checkbox.checked;    // checkbox가 type이고, checked는 속성애 해당했습니다.
    ...
    deleteBtn.addEventListener('click', () => {
      ...
      // 보통은 index로 내용을 확인하지만 계속 삭제를 하면 index 넘버가 바뀌겠조
      // 그레서 input에 입력했던 내용을 근거로 index를 역으로 찾아낼 예정입니다.
      const itemText = span.textContent;  // 얘를 쓰는 이유는 저희가 .trim() 쓰는 사람에 공백을 날렸기 때문입니다.
      // 배열 내에서의 내용과 span 토그 내에서의 내용이 같은 index를 뽑아내서 itemIndex 변수에 저장
      const itemIndex = todos.findIndex(item => item.text === itemText);

      if(itemIndex !== -1) { // 일치하는 인덱스가 없으면 -1 입니다
