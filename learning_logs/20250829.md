...
1.01_shoppinglist.css를 만들고, 01_todolist.css의 코드를 전부 다 갉어서 복사하고, 01_shoppinglist.css에서 수정한다.
  - 혹시 수정하다가 잘못건드려서 todolist의 css에 깨지면 골치아파서

2. 01_shoppinglist.css를 만드는 것가지는 좋은데, 우리가 좀 고급 css 배우면서 했던 방식을 쓴다. -> @import url(파일 경로)